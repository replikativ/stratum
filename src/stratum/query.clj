(ns stratum.query
  ...
  (:require [stratum.query.simd-primitive :as qc]
            ...
            [stratum.query.distinct :as q-distinct]
...
(defn q
  ...
  ;; Bind expr/*columns-meta*, gb/*dense-group-limit* and the
  ...
  ;; pass.
              (binding [expr/*columns-meta* (into {} (keep (fn [[k v]] (when (:dict v) [k v]))) columns)
                ...
                ;; column level via ColumnOpsExt's parallel
                ;; BitSet/Hash distinct, no row-map allocation.
                ;; Then top-N pushdown: ORDER BY single-col [DESC]
                ;; LIMIT N with no other clauses streams the order
                ;; column and fetches only the surviving N rows from
                ;; each output column.
                (cond
                  (q-distinct/distinct-eligible? query columns)
                  (q-distinct/execute-distinct query columns)

                  (top-n/top-n-eligible? query columns)
                  (top-n/execute-top-n query columns)

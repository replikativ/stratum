...
### Added
- **Bitemporal valid-time column convention** (`stratum.dataset/vt-config`, opt-in via `:metadata {:valid-time {:from-col <kw> :to-col <kw> :unit :micros}}` on `make-dataset`): tags the two named columns with `:temporal-unit :micros` so downstream date kernels dispatch correctly, validates their type at construction, and round-trips through `sync!`/`load`. Pairs with bench tier 10 (VT-Q1 1% sel, VT-Q2 50% + `Long/MAX_VALUE` sentinel, VT-Q3 group-by + vt filter) and a baseline snapshot at `bench/baseline-vt-branch.txt`. See [doc/dataset.md § Valid-Time Window](doc/dataset.md#valid-time-window).
